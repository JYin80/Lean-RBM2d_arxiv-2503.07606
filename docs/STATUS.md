...
- **基线**：主分支 `5d40557` 起步，Lean 4.34.0。两个仓库的十个依赖 commit
  ...
  RBM1D 文件始终只读。最近一次 `./check.sh` 全量通过：3997 jobs、errors 0、
  warning 0；`#assert_rbm_axioms` 报告 2185 个 `RBM` 声明，仅使用
  `propext`、`Classical.choice`、`Quot.sound`。
- **维护项**：T31 已清理 `Defs/Dist.lean` 与 `Gauss/Envelope.lean` 的十条旧
  linter warning，关键声明类型的前后编译探针一致。
- **在飞工单**：T22 的分离壳折角支持、S5 的 envelope 标签求和、T29 的
  加权逆核衰减由三个独立文件所有者推进。
  ...
  加显式分离壳余项。后者的有效求和及最终 T22 仍开放。
  T29 已有线性与指数距离权重的
  共轭扰动范数界，并在 $\|\xi\|e^t<1$ 下证明加权逆界；逐项核衰减仍开放。
  单坐标及全坐标 Stein 恒等式、
...
- **蓝图**：已补 §2–4 的 loop/卷积树、局部律、QUE、量子扩散以及
  ...
  380 条依赖；1067 个 theorem/lemma、1276 个声明中所有蓝图引用均可解析。
  全局图已按用户参考图改为宽幅白底、无章节框的依赖层布局；无 `dot` 时
  自动使用 SVG 布局。SVG 静态检查 205 节点、380 连线、0 重叠。
