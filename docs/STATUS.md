...
## 当前摘要（2026-09-22 08:23 UTC；以下旧条目是历史日志）
...
- **基线**：主分支 `5d40557` 起步，Lean 4.34.0。两个仓库的十个依赖 commit
  ...
  RBM1D 文件始终只读。最近一次 `./check.sh` 全量通过：3966 jobs、errors 0、
  warning 0；`#assert_rbm_axioms` 报告 1965 个 `RBM` 声明，仅使用
  `propext`、`Classical.choice`、`Quot.sound`。
- **维护项**：T31 已清理 `Defs/Dist.lean` 与 `Gauss/Envelope.lean` 的十条旧
  linter warning，关键声明类型的前后编译探针一致。
- **在飞工单**：T22 的折叠点 cutoff 差分、S5 的 cut RHS 连续性、S5 的
  任意长度标量初值由三个独立文件所有者推进。
...
- **蓝图**：已补 §2–4 的 loop/卷积树、局部律、QUE、量子扩散以及
  ...
  移位与未证的核衰减分别标记。最新交互 HTML 静态检查为 174 节点、
  334 条依赖；940 个 theorem/lemma、1129 个声明中所有蓝图引用均可解析。
  全局图已按用户参考图改为宽幅白底、无章节框的依赖层布局；无 `dot` 时
  自动使用 SVG 布局。SVG 静态检查 174 节点、334 连线、0 重叠。
