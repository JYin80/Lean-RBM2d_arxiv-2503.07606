...
- **基线**：主分支 `5d40557` 起步，Lean 4.34.0。两个仓库的十个依赖 commit
  ...
  RBM1D 文件始终只读。最近一次 `./check.sh` 全量通过：4006 jobs、errors 0、
  warning 0；`#assert_rbm_axioms` 报告 2261 个 `RBM` 声明，仅使用
  `propext`、`Classical.choice`、`Quot.sound`。
- **维护项**：T31 已清理 `Defs/Dist.lean` 与 `Gauss/Envelope.lean` 的十条旧
  linter warning，关键声明类型的前后编译探针一致。
- **暂停点**：本批 T22 分离壳支撑局部化、S5 长度 envelope 谱隙单调性、
  T29 谱隙参数选择均已验收。按用户要求暂停派单与定时检查，待用户通知后再继续。
  无折叠区半径二阶差分及其可判定的整数代表元充分条件、精确折角缺陷与
  cutoff 差分分解、有限 loop 的确定性初值和任意非空长度标量式、二阶 cut-loop 期望、有限期望
  层级 ODE、零时间端点在内的无附加可积性假设的时间积分式、以及初值
  的块标签支持和精确 $W$ 归一化、初值模长与二三边标签和、
  一边与二边基例、cut-loop 长度、全位置范数界和谱割环界均已验收。
  T22 的局部二阶 cutoff 界已把折角项显式隔离，低网格活跃壳只在
  距离小于八的固定盒内；支持起点至多 768，单壳绝对和至多 1536，
  活跃低网格壳至多 3，总绝对和至多 4608；有限壳层总和已分解为此界
  加显式分离壳余项。粗界每壳为 $5040\cdot2^j+3780\cdot4^j$；
  非零差分起点现位于三个物理环带平移像，数量至多
  $3(2\lceil2L2^{-j}\rceil+1)^2$；折角加权求和及最终 T22 仍开放。
  T29 已有线性与指数距离权重的
  共轭扰动范数界，并在 $\|\xi\|e^t<1$ 下证明加权逆与逐项指数核界；
  $\|\xi\|\leq1/4$ 时有具体 $2\cdot2^{-d}$ 界，
  $\|\xi\|\leq1-\delta$ 时有 $(2/\delta)e^{-\log(1+\delta/2)d}$ 界。
  完整性质 5 仍开放。
  S5 有有限时间标签和界，初值项精确，粗误差 envelope 只依赖长度，
  但带 $|Z_L^2|^n$ 因子，尚未闭合论文矩估计。
  单坐标及全坐标 Stein 恒等式、
  一个有序二边交叉项、二边反向项、单边二阶收缩及其 cut-loop 版本已验收。
  样本时间导数与坐标求和的精确关系亦已验收。T8 九点有理逆、T21a–g 逆符号
  差分及环带界、
  T28a–k 周期化代数和实际 `Kinf` 格点方程、S9c–i 连续性/包络/显式谱路径、S5 谱路径/矩阵/Green
  导数、loop 一二阶坐标导数及方差/全坐标收缩、C1 完整 loop Ward、C2 单边与双边
  矩阵词、开链闭合、谱漂移切接及坐标导数可积性均已本地全量验收。
  T21 的 `Shat` 一至三阶差分已验收，逆乘子完整环带界仍开放。
  T20 周期求和、S9a 通用 Grönwall 引理、T19、T27 第一坐标阶段、S7、S8、T6、T31
  与 S9b 保尺度收口已在本地全量验收；完整 T27 核衰减及 S9 实际矩估计仍开放。
  不再新建项目对话条。
  协调任务持有共享文档与蓝图。
  T3 原规格的 `3/π²`
  与现有粗壳层界不直接相符，
...
- **蓝图**：已补 §2–4 的 loop/卷积树、局部律、QUE、量子扩散以及
  ...
  移位与未证的核衰减分别标记。最新交互 HTML 静态检查为 193 节点、
  390 条依赖；1112 个 theorem/lemma、1327 个声明中所有蓝图引用均可解析。
  全局图已按用户参考图改为宽幅白底、无章节框的依赖层布局；无 `dot` 时
  自动使用 SVG 布局。SVG 静态检查 214 节点、390 连线、0 重叠。
...
- **T22 cutoff 边界**：`normalizedFrequency` 基于周期角距离，在对踵点
  有折角。仅凭正半径不能推出其二阶差分为 $O(L^{-2})$；已编译的
  $L=8$ 对踵三点 $(3,0),(4,0),(5,0)$ 给出 $2/L=1/4$ 的反例。
  后续 sharp cutoff 估计须显式避开折叠或改用平滑的周期分解。
...
- **S4 集成（2026-09-22）**：`Gauss/Model.lean` 已从独立工作树接入。
  有限维 Hermitian 高斯模型证明条目方差与 `Spaper` 对应；共同概率空间
  `Sizes.seqP` 的每个尺寸投影等于有限维 `P L W`，并有非零对角方差例子。
  主分支全量 `./check.sh` 通过：3668 jobs、errors 0；762 个 `RBM`
  声明只用标准三项公理。蓝图节点已标绿。
- **T25 与交互蓝图（2026-09-22）**：`LogIntegral.lean` 证明论文
  `(eq_log_int)` 的二维方形积分界，显式常数 32，$\kappa=1$ 可满足性
  例子已编译。全量 `./check.sh` 通过：3669 jobs、errors 0；771 个
  `RBM` 声明只用标准三项公理。`blueprint/web/artifact.html` 已由
  `render_artifact.py` 生成，HTML 解析验证七张 SVG 图均存在。
- **S3 实际矩阵桥（2026-09-22）**：`SteinConcrete.lean` 将通用乘积高斯
  Stein 恒等式实例化到真实 Hermitian 矩阵和一时刻流，证明坐标重采样、
  更新方向与两个固定时刻版本；对角正方差 $1/5$ 的非恒定正弦测试已编译。
  全量 `./check.sh` 通过：3670 jobs、errors 0；778 个 `RBM` 声明
  只用标准三项公理。S5 的时间微分与方向合并仍开放。
- **T26 集成（2026-09-22）**：`ContinuumSymbol.lean` 构造连续符号与
  真正可积的无穷体积 Fourier 核，证明基本方形上的双边椭圆界及
  迭代积分与二维 Lebesgue 积分一致；$K_{0,\infty}(0)=1$ 是非空探针。
  全量 `./check.sh` 通过：3671 jobs、errors 0；818 个 `RBM` 声明
  只用标准三项公理。蓝图 T26 节点已标绿，T27/T28 接口可用。
- **S6 集成（2026-09-22）**：`Gauss/Domination.lean` 从偶数阶矩界证明
  Def. 2.1 的随机支配，包含按实际矩阵维数的版本与连续时间网格。
  网格的 Hölder 模数只在高概率好事件上要求，避免给高斯样本路径
  强加不可能的全空间确定性上界。全量 `./check.sh` 通过：3672 jobs、
  errors 0；849 个 `RBM` 声明只用标准三项公理。实际矩估计和
  高斯流模数的好事件仍由后续工单提供。
- **T24 规格修订（2026-09-22）**：既然 T25 已证明真实对数积分，
  T24 不再预留 `LogIntegral`/`ContourInput` 的自由证明字段；待 T27/T28
  产出实际定理后直接合并两区制。蓝图依赖边相应更新，交互图已重生成。
- **T19/S7/S8/T27 阶段/T6/T31 集成（2026-09-22）**：显式 $C^3$ dyadic
  cutoff 已证三阶差分界及归一化区间上的精确分解；逆向随机支配桥在全局
  多项式包络与正尺度下界下给出偶矩界；连续归纳有逐点、前缀和 $2B$
  形式；围道已证第一坐标的零点排除及 Cauchy 移位，最终核衰减仍开放；
  $(\mathrm{deri}_{\Theta\xi})$ 已按 d=1 只读移植为 d=2 逐元素导数公式。
  10 条旧 linter warning 已清，关键声明类型不变。全量 `./check.sh`：
  3681 jobs、0 error、0 warning；966 个 `RBM` 声明只用标准三项公理。
  蓝图把阶段结果与开放总目标分开标记。
- **T20/S9a 集成（2026-09-22）**：周期分部求和与相位损失双边界
  已证；Mathlib 的通用单侧导数 Grönwall 桥也已证。主分支 `./check.sh`
  全量通过：3684 jobs、0 error、0 warning；993 个 `RBM` 声明只用标准三项公理。
  d=1 的 `Analysis/MomentClosing.lean` 明言粗 Grönwall 会损失 $\eta$ 的幂；
  对照 d=2 论文 §5 的二次变差估计后，已确认通用桥不能单独保持所需尺度。
- **S9b 集成（2026-09-22）**：从 d=1 只读移植完整的积分—上确界—二次收口链，
  在诚实的非负、可积、`IsLUB` 与积分不等式前提下保留
  $\int f+\sqrt{\int g}$ 的尺度；$\psi(u)=1+u$ 的非恒定探针已编译。
  全量 `./check.sh`：3685 jobs、0 error、0 warning；1000 个 `RBM`
  声明只用标准三项公理。S5/S10 尚须给出具体矩函数的积分不等式和二次变差对应。
- **S9b 上界包装/T21 阶段验收（2026-09-22）**：连续时间窗口由紧性得到
  上界，再应用保尺度矩收口；二维真实 Fourier 符号的前三阶坐标差分有
  显式 $r/L+L^{-2}$、$L^{-2}$、$L^{-3}$ 界。全量 `./check.sh`：
  3687 jobs、0 error、0 warning；1012 个 `RBM` 声明只用标准三项公理。
  T21 的逆乘子与环内移位尚未证明。
- **T8/T28a/S9c/C1 基础包（2026-09-22）**：T8 在 $L=3,\xi=1/2$ 下用
  独立五点核和精确有理矩阵证全 $9\times9$ 双侧逆、与 `Theta` 一致及行和 2；
  T28a 证 $\mathbb Z^2$ 正半径最大范数壳层恰有 $8r$ 点；S9c 由统一包络和
  支配收敛证矩的时间连续性，并证固定样本的矩阵流与移动非实谱参数 resolvent
  连续；C1 证 loop 定义、块求和系数 $W^{-2}$、两环条目系数 $W^{-4}$。
  全量 `./check.sh`：3871 jobs、0 error、0 warning；1144 个 `RBM` 声明只用
  标准三项公理。具体 loop 连续性、T28 无限尾和 Ward 一般式仍开放。
- **Ward 论文修订（2026-09-22）**：Jun 裁定 `(WI_calL)`、`(WI_calK)` 与说明行
  缺虚数单位；已把论文系数改为 $1/(2\mathrm{i}W^2\eta_t)$，蓝图同步，
  `docs/paper-deltas.md` 第 27 项记载依据。两环 Lean 证明在做。
