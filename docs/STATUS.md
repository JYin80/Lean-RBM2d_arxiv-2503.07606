...
## 当前摘要（2026-09-22 06:09 UTC；以下旧条目是历史日志）
...
- **基线**：主分支 `5d40557` 起步，Lean 4.34.0。两个仓库的十个依赖 commit
  ...
  RBM1D 文件始终只读。最近一次 `./check.sh` 全量通过：3687 jobs、errors 0、
  warning 0；`#assert_rbm_axioms` 报告 1012 个 `RBM` 声明，仅使用
  `propext`、`Classical.choice`、`Quot.sound`。
- **维护项**：T31 已清理 `Defs/Dist.lean` 与 `Gauss/Envelope.lean` 的十条旧
  linter warning，关键声明类型的前后编译探针一致。
- **在飞工单**：T8 数值回归与 S9 时间连续性辅助引理正在做；C1 的 loop
  定义与块求和也已分段派发。T21 的 `Shat` 一至三阶差分阶段已验收，
  逆乘子差分仍开放。
...
- **蓝图**：已补 §2–4 的 loop/卷积树、局部律、QUE、量子扩散以及
  ...
  移位与未证的核衰减分别标记。最新交互 HTML 静态检查 78 节点、
  138 条依赖，另有六张章节图；无 `dot` 时自动使用 SVG 布局。
