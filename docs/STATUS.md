...
- **基线**：主分支 `5d40557` 起步，Lean 4.34.0。两个仓库的十个依赖 commit
  ...
  RBM1D 文件始终只读。最近一次 `./check.sh` 全量通过：3684 jobs、errors 0、
  warning 0；`#assert_rbm_axioms` 报告 993 个 `RBM` 声明，仅使用
  `propext`、`Classical.choice`、`Quot.sound`。
- **维护项**：T31 已清理 `Defs/Dist.lean` 与 `Gauss/Envelope.lean` 的十条旧
  linter warning，关键声明类型的前后编译探针一致。
- **在飞工单**：T8 数值回归正在做；T21 单步乘子差分在做。
  T20 周期求和、S9a 通用 Grönwall 引理、T19、T27 第一坐标阶段、S7、S8、T6、T31
  已在本地全量验收；完整 T27 核衰减及 S9 实际矩估计仍开放。
  不再新建项目对话条。
  协调任务持有共享文档与蓝图。
  T3 原规格的 `3/π²`
  与现有粗壳层界不直接相符，
...
- **蓝图**：已补 §2–4 的 loop/卷积树、局部律、QUE、量子扩散以及
  ...
  移位与未证的核衰减分别标记。最新交互 HTML 静态检查 76 节点、
  135 条依赖，另有六张章节图；无 `dot` 时自动使用 SVG 布局。
...
- **T20/S9a 集成（2026-09-22）**：周期分部求和与相位损失双边界
  ...
  S9 在 d=2 是否可直接使用通用桥正按论文实际不等式核对。
