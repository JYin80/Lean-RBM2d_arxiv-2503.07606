...
## 当前摘要（2026-09-22 06:59 UTC；以下旧条目是历史日志）
...
- **基线**：主分支 `5d40557` 起步，Lean 4.34.0。两个仓库的十个依赖 commit
  ...
  RBM1D 文件始终只读。最近一次 `./check.sh` 全量通过：3893 jobs、errors 0、
  warning 0；`#assert_rbm_axioms` 报告 1299 个 `RBM` 声明，仅使用
  `propext`、`Classical.choice`、`Quot.sound`。
- **维护项**：T31 已清理 `Defs/Dist.lean` 与 `Gauss/Envelope.lean` 的十条旧
  linter warning，关键声明类型的前后编译探针一致。
- **在飞工单**：T28 的实际 Fourier 被积函数恒等式、S5 的 Green 流导数、
  C2/S5 的生成元收缩基础由三个独立文件所有者推进。T8 九点有理逆、
  T28a–j 壳层至条件 resolvent 桥及 Fourier 点质量、S9c–i 连续性、包络和
  显式谱路径、S5 谱路径/矩阵流导数、C1 完整 loop Ward、C2 单边与双边
  矩阵词均已本地全量验收。
  T21 的 `Shat` 一至三阶差分已验收，逆乘子差分仍开放。
...
- **蓝图**：已补 §2–4 的 loop/卷积树、局部律、QUE、量子扩散以及
  ...
  移位与未证的核衰减分别标记。最新交互 HTML 静态检查为 104 节点、
  204 条依赖；662 个 theorem/lemma、785 个声明中所有蓝图引用均可解析。
  无 `dot` 时自动使用 SVG 布局。
