...
## 当前摘要（2026-09-22 07:32 UTC；以下旧条目是历史日志）
...
- **基线**：主分支 `5d40557` 起步，Lean 4.34.0。两个仓库的十个依赖 commit
  ...
  RBM1D 文件始终只读。最近一次 `./check.sh` 全量通过：3917 jobs、errors 0、
  warning 0；`#assert_rbm_axioms` 报告 1510 个 `RBM` 声明，仅使用
  `propext`、`Classical.choice`、`Quot.sound`。
- **维护项**：T31 已清理 `Defs/Dist.lean` 与 `Gauss/Envelope.lean` 的十条旧
  linter warning，关键声明类型的前后编译探针一致。
- **在飞工单**：T21 的环带分子收口、S5 的全坐标 Stein 求和、C2/S5 的
  反向二边交叉项由三个独立文件所有者推进。单坐标 Stein 恒等式与一个
  有序二边交叉项已验收。T8 九点有理逆、T21a–f 逆符号
  差分及环带界、
...
- **蓝图**：已补 §2–4 的 loop/卷积树、局部律、QUE、量子扩散以及
  ...
  移位与未证的核衰减分别标记。最新交互 HTML 静态检查为 126 节点、
  254 条依赖；756 个 theorem/lemma、898 个声明中所有蓝图引用均可解析。
  无 `dot` 时自动使用 SVG 布局。
