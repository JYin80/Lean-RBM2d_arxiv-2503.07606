...
## 当前摘要（2026-09-22 08:18 UTC；以下旧条目是历史日志）
...
- **基线**：主分支 `5d40557` 起步，Lean 4.34.0。两个仓库的十个依赖 commit
  ...
  RBM1D 文件始终只读。最近一次 `./check.sh` 全量通过：3960 jobs、errors 0、
  warning 0；`#assert_rbm_axioms` 报告 1904 个 `RBM` 声明，仅使用
...
- **在飞工单**：T22 的折叠点 cutoff 差分、S5 的 cut RHS 连续性、S5 的
  ...
  无折叠区半径二阶差分及其可判定的整数代表元充分条件、精确折角缺陷、
  有限 loop 的确定性初值和一二边标量式、二阶 cut-loop 期望、有限期望
  层级 ODE 与条件版时间积分式已验收。条件版积分式仍需证明实际 cut RHS
  在紧时间区间的可积性或连续性。
  单坐标及全坐标 Stein 恒等式、
...
- **蓝图**：已补 §2–4 的 loop/卷积树、局部律、QUE、量子扩散以及
  ...
  移位与未证的核衰减分别标记。最新交互 HTML 静态检查为 168 节点、
  326 条依赖；906 个 theorem/lemma、1089 个声明中所有蓝图引用均可解析。
  全局图已按用户参考图改为宽幅白底、无章节框的依赖层布局；无 `dot` 时
  自动使用 SVG 布局。SVG 静态检查 168 节点、326 连线、0 重叠。
