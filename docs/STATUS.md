...
## 当前摘要（2026-09-22 07:35 UTC；以下旧条目是历史日志）
...
- **基线**：主分支 `5d40557` 起步，Lean 4.34.0。两个仓库的十个依赖 commit
  ...
  RBM1D 文件始终只读。最近一次 `./check.sh` 全量通过：3920 jobs、errors 0、
  warning 0；`#assert_rbm_axioms` 报告 1524 个 `RBM` 声明，仅使用
  `propext`、`Classical.choice`、`Quot.sound`。
- **维护项**：T31 已清理 `Defs/Dist.lean` 与 `Gauss/Envelope.lean` 的十条旧
  linter warning，关键声明类型的前后编译探针一致。
- **在飞工单**：T21 的低频层、S5 的样本时间导数与坐标求和对应、C2/S5
  的单边二阶项由三个独立文件所有者推进。单坐标及全坐标 Stein 恒等式、
  一个有序二边交叉项及二边反向项已验收。T8 九点有理逆、T21a–g 逆符号
  差分及环带界、
...
- **蓝图**：已补 §2–4 的 loop/卷积树、局部律、QUE、量子扩散以及
  ...
  移位与未证的核衰减分别标记。最新交互 HTML 静态检查为 129 节点、
  258 条依赖；764 个 theorem/lemma、906 个声明中所有蓝图引用均可解析。
  无 `dot` 时自动使用 SVG 布局。
